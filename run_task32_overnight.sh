...
LOG_FILE="logs/task32_${JOB_ID}_${TS}.log"
SUMMARY_FILE="logs/task32_${JOB_ID}_${TS}_summary.txt"
...
echo "=== $(date '+%F %T') Task32(L2 Adaptive) 시작 ==="
...
ADAPT_NORM_DIR="../DCASE2024_SELD_dataset/seld_feat_label/foa_dev_norm_l2foa_adapt"

DO_FEAT=1
if [[ "${RUN_FEAT}" == "0" ]]; then
    DO_FEAT=0
elif [[ "${RUN_FEAT}" == "auto" ]]; then
    if [[ -d "${ADAPT_NORM_DIR}" ]] && find "${ADAPT_NORM_DIR}" -name '*.npy' -print -quit | grep -q .; then
        DO_FEAT=0
        echo "적응형 L2 정규화 feature가 이미 존재하여 Step1을 생략합니다: ${ADAPT_NORM_DIR}"
    fi
fi

if [[ "${DO_FEAT}" == "1" ]]; then
    echo "=== $(date '+%F %T') Step 1: Feature Extraction (task 32) ==="
    stdbuf -oL -eL python3 -u -X faulthandler batch_feature_extraction.py "${TASK_ID}"
else
    echo "=== $(date '+%F %T') Step 1: Feature Extraction 생략 ==="
fi

echo "=== $(date '+%F %T') Step 2: Train + Test Eval (task 32, job ${JOB_ID}) ==="
