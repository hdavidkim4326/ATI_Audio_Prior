...
TASK_ID=33
JOB_ID="${JOB_ID:-decoupled33}"
GPU_ID="${GPU_ID:-1}"
RUN_FEAT="${RUN_FEAT:-auto}" # auto | 1 | 0

mkdir -p logs
TS="$(date '+%Y%m%d_%H%M%S')"
LOG_FILE="logs/task33_${JOB_ID}_${TS}.log"
SUMMARY_FILE="logs/task33_${JOB_ID}_${TS}_summary.txt"
...
echo "=== $(date '+%F %T') Task33(Decoupled Adaptive L2) 시작 ==="
...
DECOUPLED_NORM_DIR="../DCASE2024_SELD_dataset/seld_feat_label/foa_dev_norm_l2foa_decoupled"

DO_FEAT=1
if [[ "${RUN_FEAT}" == "0" ]]; then
    DO_FEAT=0
elif [[ "${RUN_FEAT}" == "auto" ]]; then
    if [[ -d "${DECOUPLED_NORM_DIR}" ]] && find "${DECOUPLED_NORM_DIR}" -name '*.npy' -print -quit | grep -q .; then
        DO_FEAT=0
        echo "Decoupled 정규화 feature가 이미 존재하여 Step1을 생략합니다: ${DECOUPLED_NORM_DIR}"
    fi
fi

if [[ "${DO_FEAT}" == "1" ]]; then
    echo "=== $(date '+%F %T') Step 1: Feature Extraction (task 33) ==="
    stdbuf -oL -eL python3 -u -X faulthandler batch_feature_extraction.py "${TASK_ID}"
else
    echo "=== $(date '+%F %T') Step 1: Feature Extraction 생략 ==="
fi

echo "=== $(date '+%F %T') Step 2: Train + Test Eval (task 33, job ${JOB_ID}) ==="
